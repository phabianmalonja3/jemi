...
async function fetchAppDetails(packageName: string): Promise<AppDetailsResponse> {
  const baseUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8080/api/v0.1";
...
export default async function AppDetailPage({ params }: PageProps) {
  ...
  const packageName = decodeURIComponent(resolvedParams.packageName).trim();

  // Ita API kupitia function yetu salama
  const data = await fetchAppDetails(packageName);

  // Hesabu jumla ya tracks na matoleo (releases) kutoka kwenye API response
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-6">
        ...
        {/* Orodha ya Tracks na Releases kutoka Backend */}
